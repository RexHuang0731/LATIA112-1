...
def azure_sentiment(user_input):
    ...
    response2 = text_analytics_client.extract_key_phrases(documents = documents)[0]
    ...
    for idx, doc in enumerate(docs):
        ...
        if doc.sentiment == "positive":
            sentiment_result = f"正向 {doc.confidence_scores.positive:.2f}"
        elif doc.sentiment == "negative":
            sentiment_result = f"負向 {doc.confidence_scores.negative:.2f}"
        else:
            sentiment_result = f"中性 {doc.confidence_scores.neutral:.2f}"
    
    sentiment_result = f"{a}\n{sentiment_result}"
